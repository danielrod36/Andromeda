...
Cepheus Adventure (repo: "Andromeda") — a single-player CYOA RPG where a **deterministic rules engine** owns dice, state, and outcomes, and an **LLM only narrates**. Built on Cepheus Engine (2D6 sci-fi TTRPG) mechanics with pluggable theme packs.
...
uv run cepheus-adventure             # equivalent (installed console script)
```
...
Work happens on feature branches off `main`. Commits reference plan/audit codes (U1–U8 = units, R1–R24 = requirements, AE1–AE13 = acceptance/edge cases, AExx); these appear in docstrings and test names. `docs/plans/` and `docs/research/cepheus-adventure/` hold the design rationale and a CE SRD rules audit.
