...
**Kilo Code Review** (GitHub App, no repo config): auto-triggers on every PR push — never request/re-run it manually. The account uses a BYOK API key from a different provider, so reviews cost nothing. History: runs on 2026-08-24 failed with "insufficient credits" because the wrong provider was configured (metering instead of BYOK); fixed same day — if that error ever recurs, first check the provider selection in the Kilo dashboard (follow the check's details URL).
